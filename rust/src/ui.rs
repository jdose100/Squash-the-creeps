...
static EN_LANGUAGE: LazyLock<LanguageText> = LazyLock::new(|| { 
    LanguageText {
...
static RU_LANGUAGE: LazyLock<LanguageText> = LazyLock::new(|| { 
    LanguageText {
...
pub struct UserInterface {
    #[init(val = &RU_LANGUAGE)] /// The current language.
    current_language: &'static LanguageText,

    #[init(val = 0)] /// Maximum score in current game.
    max_score: i64,

    #[init(val = 0)] /// Current score while player alive.
    score: i64,

    base: Base<Control>
}

#[godot_api] impl IControl for UserInterface {
    fn ready(&mut self) {
        // update UI to default language
        self.update_text_from_language();

        // connect 'pressed' signal for language button
        self.base().get_node_as::<Button>("MainHUD/LanguageButton")
            .signals().pressed()
            .connect_obj(self, Self::on_language_button_pressed);
    }
}

#[godot_api] impl UserInterface {
    /// Setup interface to new language.
    fn update_text_from_language(&mut self) {
        let language = self.current_language;

        // update max score
        self.base().get_node_as::<Label>("MaxScoreLabel").set_text(
            &format!("{}: {}", language.max_score, self.max_score)
        );

        // update score
        self.base().get_node_as::<Label>("ScoreLabel").set_text(
            &format!("{}: {}", language.score, self.score)
        );

        // update language button
        self.base().get_node_as::<Button>("MainHUD/LanguageButton").set_text(
            &format!("{} (ru/en)", language.language_button)
        );

        // update start button
        self.base().get_node_as::<Button>("MainHUD/StartButton").set_text(
            language.start_button
        );

        // update text logo
        self.base().get_node_as::<Label>("MainHUD/NameOfGame").set_text(
            language.name_of_game
        );

        // update positions for text
        match self.current_language.language {
            Languages::EN => 
                self.base().get_node_as::<Label>("MainHUD/NameOfGame")
    ...
    pub fn start_new_game(&mut self) {
        // set score to zero
        self.base().get_node_as::<Label>("ScoreLabel").set_text(
            &format!("{}: 0", self.current_language.score)
        );
        self.score = 0;
    ...
    pub fn set_improvement(
        ...
        // update UI
        let mut improvement_label = 
            self.base().get_node_as::<Label>("Improvement");
        improvement_label.show();

        match improvement {
            Improvements::SlowCreeps => {
                improvement_label.set_text(
                    self.current_language.improvement_slow_creeps
                );
            },
            Improvements::PlayerShield => {
                improvement_label.set_text(
                    self.current_language.improvement_shield
                );
            },
    ...
    pub fn on_mob_squashed(&mut self) {
        ...
        self.score += 1;
        self.base().get_node_as::<Label>("ScoreLabel").set_text(
            &format!("{}: {}", self.current_language.score, self.score)
        );
        ...
        if self.score > self.max_score {
            ...
            self.base().get_node_as::<Label>("MaxScoreLabel").set_text(
                &format!("{}: {}", self.current_language.max_score, self.max_score)
            );
        }
    }
